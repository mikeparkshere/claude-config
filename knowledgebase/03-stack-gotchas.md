...
**First seen:** TAB, 2026-06-26 — pre-launch CSS-handoff sweep; CLI switch left global classes inline, reverted to baseline, deferred to cutover.
...
### Bricks — typed settings written via a `bricks_global_classes` CLI write emit for some control types, silently not for others
**Symptom / When:** Converting global-class `_cssCustom` to typed settings by a direct `bricks_global_classes` option write + `Assets_Files::regenerate_css_files()`. Some typed keys produce CSS; others persist in the option but emit nothing — no error, and readback looks correct.
...
**⚠️ Scope + open conflict — flagged at the 2026-07-15 VMG harvest, NOT yet resolved.** This was found on the **global-class option-write** path, which is a *different emitter* from element-typed settings written into `_bricks_page_content_2` (see "where CSS actually lives: global-class CSS is INLINE, element-typed CSS is in `post-{id}.min.css`"). `02`'s verified typed-settings library lists `_columnGap`/`_rowGap`, `_width`, `_widthMax` and `_gridTemplateColumns` as working — captured from TAB's element-tree builds on Bricks 2.3.8. Both can be true if the no-emit is specific to the global-class path and/or to 2.3.6. **Untested against each other.** Until someone tests it: on the global-class path, verify every typed write against the rendered page; treat `02`'s library as element-path-verified.
**First seen:** VMG, 2026-06-06 — a Contact page typed conversion; gap / flex-direction / max-width dropped silently.
